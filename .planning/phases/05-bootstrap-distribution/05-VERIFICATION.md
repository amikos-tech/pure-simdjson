...
status: verified
score: 10/10 must-haves verified; live public bootstrap covered by Phase 06.1 hosted UAT; corporate-network case remains backlog-tracked
overrides_applied: 0
human_verification: []
---
...
**Verified:** 2026-04-23T12:29:00Z
**Status:** verified
**Re-verification:** Yes — live public bootstrap UAT completed via Phase 06.1 hosted validation
...
**Fresh-machine end-to-end bootstrap against live infrastructure — passed via Phase 06.1**

**Test:** Dispatch `.github/workflows/public-bootstrap-validation.yml` with `version=0.1.0` and validate the public bootstrap path on hosted runners.

**Expected:** Artifact downloads from `releases.amikos.tech`, SHA-256 verifies against published `SHA256SUMS`, caches to OS user-cache-dir with 0700 perms on unix, and parser opens successfully.

**Evidence:** GitHub Actions run `24835017953` completed successfully on 2026-04-23. The `validate-r2` matrix passed all five targets (`linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64`, `windows/amd64`) and the `validate github fallback` subset passed `linux/amd64`, `darwin/arm64`, and `windows/amd64`.

**Corporate-firewall workaround against a real proxy — still backlog-tracked**
...
**Status:** Not a Phase 5 blocker. This remains parked as backlog Phase `999.5` because it requires a real corporate-network environment or proxy emulation.
...
**Remaining environmental item:** Corporate-firewall verification is backlog-tracked as Phase `999.5`. The live-endpoint bootstrap path is now covered by Phase `06.1` hosted UAT.

---

_Verified: 2026-04-23T12:29:00Z_
