...
- `gh auth status` returned non-zero because of an unrelated timeout on `github.teliacompany.net`; repository-host verification was instead done with `gh auth status -h github.com`, which succeeded.
