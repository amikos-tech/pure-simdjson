...
- [x] Extended release/bootstrap policy tests for version `0.1.5` and ABI 1.2.
...
|----------|-------------|------------|-------------------|
| Select and approve the intermediate ABI 1.2 artifact semantic version | D-17 | Operator authority was required | Resolved by `11-01`: version `0.1.5` is recorded in the summary, bootstrap pin, ABI policy, and changelog without consuming Phase 16's final v0.2 label. |
| Publish matching ABI 1.2 artifacts through the supported path | UP-01, D-17, D-18 | Requires `origin/main`, an annotated tag, hosted runners, signing, and external publication | `11-14-T1` must run `bash scripts/release/check_readiness.sh --strict --version 0.1.5` on the approved `main` commit, create/push the annotated tag, and require `.github/workflows/release.yml` to pass. Never hand-upload artifacts. |
| Prove fresh-runner default bootstrap after publication | D-17, D-18 | Requires live R2/GitHub release assets and hosted target runners | `11-14-T1` must dispatch `.github/workflows/public-bootstrap-validation.yml` for `0.1.5` and require the full R2 matrix plus documented GitHub fallback subset to pass. |
