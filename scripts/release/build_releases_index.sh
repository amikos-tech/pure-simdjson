...
usage() {
  cat <<'EOF'
Usage:
  build_releases_index.sh --output <path> --project <name> --version <tag> --date <utc-timestamp> [--existing <path>] [--domain <host>] [--max <n>]
...
if [[ -n "$releases_domain" ]]; then
  new_entry="$(jq -n \
    ...
    --arg checksums_full_url "https://${releases_domain}/${checksums_url}" \
    '{version: $version, date: $date, prefix: $prefix, checksums_url: $checksums_url, checksums_full_url: $checksums_full_url}')"
else
  new_entry="$(jq -n \
    --arg version "$version" \
    --arg date "$release_date" \
...
def normalize:
  if type != "object" then
  ...
  else
    {
      ...
      checksums_full_url: (.checksums_full_url // null)
    }
    | select(
